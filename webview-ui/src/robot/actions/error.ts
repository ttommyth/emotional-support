...
export const error: RobotActionDefinition = {
	...
	apply: (t, { targets }) => {
		...
		const shake = shaking ? Math.sin(t * 18) * 0.06 * (1 - phase / 1.2) : 0;

		targets.body.pos.x = shake;
		targets.body.pos.y = -0.15 + Math.sin(t * 1.2) * 0.04;
		...
		targets.leftArm.pos.y = 1.5 + (shaking ? Math.sin(t * 12) * 0.06 : 0);
		targets.rightArm.pos.y = 1.5 + (shaking ? Math.cos(t * 12) * 0.06 : 0);

		// Head droops between shakes, shakes during burst
		if (shaking) {
			targets.head.rot.y = Math.sin(t * 14) * 0.15;
			targets.head.rot.x = -0.1 + Math.sin(t * 10) * 0.05;
		} else {
			// Slow frustrated sway
			targets.head.rot.x = 0.2 + Math.sin(t * 0.8) * 0.06;
			targets.head.rot.y = Math.sin(t * 0.6) * 0.25;
			targets.head.rot.z = Math.sin(t * 0.9) * 0.05;
		}
	}
};
