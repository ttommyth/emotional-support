...
export const sit: RobotActionDefinition = {
	...
	pre: {
		...
		apply: (p, _t, { targets }) => {
			...
			targets.leftArm.rot.set(-0.2 * eased, 0, 0.15 * eased);
			targets.rightArm.rot.set(-0.2 * eased, 0, -0.15 * eased);
	...
	apply: (t, { targets }) => {
		...
		targets.leftArm.rot.set(-0.25 + Math.sin(t * 1.4) * 0.05, 0, 0.2);
		targets.rightArm.rot.set(-0.25 - Math.sin(t * 1.4) * 0.05, 0, -0.2);
	...
	post: {
		...
		apply: (p, _t, { targets }) => {
			...
			targets.leftArm.rot.set(-0.2 * (1 - eased), 0, 0.15 * (1 - eased));
			targets.rightArm.rot.set(-0.2 * (1 - eased), 0, -0.15 * (1 - eased));
