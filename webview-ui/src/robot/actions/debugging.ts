...
export const debugging = defineAction({
	...
	apply: (t, ctx) => {
		...
		targets.body.rot.x = 0.06 + Math.sin(t * 0.8) * 0.02 * T;
		...
		if (phase < 2) {
			...
			const p = (phase - 2) / 1.5;
			targets.head.rot.x = -0.1 - p * 0.08 * T;
			targets.head.rot.y = 0.3 - p * 0.15;
			targets.head.rot.z = -0.06 * Math.sin(p * Math.PI) * T;
			targets.rightArm.rot.set(-0.85 + Math.sin(p * 6) * 0.08 * T, 0.25, 0.8);
