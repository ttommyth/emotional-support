...
export const stretch: RobotActionDefinition = {
	...
	apply: (t, { targets }) => {
		...
		if (phase < 2) {
			...
			targets.leftArm.rot.set(-1.6 - p * 0.6, -0.15, -0.35 - p * 0.15);
			targets.rightArm.rot.set(-1.6 - p * 0.6, 0.15, 0.35 + p * 0.15);
			...
			targets.leftArm.rot.set(-2.2, -0.2, -0.5 - Math.sin(p * Math.PI) * 0.1);
			targets.rightArm.rot.set(-2.2, 0.2, 0.5 + Math.sin(p * Math.PI) * 0.1);
			targets.head.rot.x = -0.1 + Math.sin(p * Math.PI) * 0.03;
		} else {
			const p = (phase - 4) / 2;
			targets.leftArm.pos.y = 1.25 - p * 0.15;
			targets.rightArm.pos.y = 1.25 - p * 0.15;
			targets.leftArm.rot.set(-2.2 + p * 0.8, -0.18, -0.5 + p * 0.12);
			targets.rightArm.rot.set(-2.2 + p * 0.8, 0.18, 0.5 - p * 0.12);
